...
{
public:
    explicit SCD4x(scd4x_sensor_type_e sensorType = SCD4x_SENSOR_SCD40);

    // Please see the code examples for an explanation of what measBegin, autoCalibrate and skipStopPeriodicMeasurements do
    bool begin(bool measBegin) { return begin(i2c0, measBegin); }
    bool begin(bool measBegin, bool autoCalibrate) { return begin(i2c0, measBegin, autoCalibrate); }
    bool begin(bool measBegin, bool autoCalibrate, bool skipStopPeriodicMeasurements) { return begin(i2c0, measBegin, autoCalibrate, skipStopPeriodicMeasurements); }
    bool begin(bool measBegin, bool autoCalibrate, bool skipStopPeriodicMeasurements, bool pollAndSetDeviceType) { return begin(i2c0, measBegin, autoCalibrate, skipStopPeriodicMeasurements, pollAndSetDeviceType); }
    bool begin(i2c_inst_t *i2cInst = i2c0, bool measBegin = true, bool autoCalibrate = true, bool skipStopPeriodicMeasurements = false, bool pollAndSetDeviceType = true); //By default use Wire port

    bool startPeriodicMeasurement(); // Signal update interval is 5 seconds
    ...
    bool stopPeriodicMeasurement(uint16_t delayMillis = 500, i2c_inst_t *i2cInst = i2c0);

    bool readMeasurement(); // Check for fresh data; store it. Returns true if fresh data is available

    uint16_t getCO2(); // Return the CO2 PPM. Automatically request fresh data is the data is 'stale'
    float getHumidity(); // Return the RH. Automatically request fresh data is the data is 'stale'
    float getTemperature(); // Return the temperature. Automatically request fresh data is the data is 'stale'
    ...
    float getTemperatureOffset(); // Will return zero if offset is invalid
    ...
    uint16_t getSensorAltitude(); // Will return zero if altitude is invalid
    ...
    bool startLowPowerPeriodicMeasurement(); // Start low power measurements - receive data every 30 seconds
    bool getDataReadyStatus(); // Returns true if fresh data is available
    ...
    bool performSelfTest(); // Takes 10 seconds to complete. Returns true if the test is successful
    ...
    bool measureSingleShot(); // SCD41 only. Request a single measurement. Data will be ready in 5 seconds
    bool measureSingleShotRHTOnly(); // SCD41 only. Request RH and T data only. Data will be ready in 50ms
...
private:
    //Variables
    i2c_inst_t *_i2cPort = nullptr;                                                                       //The generic connection to user's chosen I2C hardware
