...
bool SCD4x::begin(uint8_t i2cInst, bool measBegin, bool autoCalibrate, bool skipStopPeriodicMeasurements,
...
bool SCD4x::stopPeriodicMeasurement(uint16_t delayMillis, uint8_t i2cInst) {
    uint8_t buf[2];
    buf[0] = SCD4x_COMMAND_STOP_PERIODIC_MEASUREMENT >> 8;   //MSB
    buf[1] = SCD4x_COMMAND_STOP_PERIODIC_MEASUREMENT & 0xFF; //LSB
    int i2cResult = I2C_SendMsg(i2cInst, SCD4x_ADDRESS, buf, 2, false, TIMEOUT_MS * 1000);
...
bool SCD4x::readMeasurement() {
    ...
    int i2cResult = I2C_SendMsg(_i2cPort, SCD4x_ADDRESS, buf, 2, false, TIMEOUT_MS * 1000);

    if (i2cResult != 2)
        return false; // Sensor did not ACK


    WaitMs(1); //Datasheet specifies this

    WaitMs(1); // Datasheet specifies this

    uint8_t incoming[9];
    I2C_RecvMsg(_i2cPort, SCD4x_ADDRESS, incoming, 9, false, TIMEOUT_MS * 1000);
...
bool SCD4x::performForcedRecalibration(uint16_t concentration, float *correction) {
    ...
    int i2cResult = I2C_SendMsg(_i2cPort, SCD4x_ADDRESS, command, 3, false, TIMEOUT_MS * 1000);
    if (i2cResult != 3)
        return false; // Sensor did not ACK

    WaitMs(400); // Datasheet specifies this

    uint8_t incoming[3];
    I2C_RecvMsg(_i2cPort, SCD4x_ADDRESS, incoming, 3, false, TIMEOUT_MS * 1000);
...
bool SCD4x::getSerialNumber(char *serialNumber) {
    ...
    int i2cResult = I2C_SendMsg(_i2cPort, SCD4x_ADDRESS, command, 2, false, TIMEOUT_MS * 1000);
    if (i2cResult != 2)
        return false; // Sensor did not ACK
...
bool SCD4x::sendCommand(uint16_t command, uint16_t arguments) {
    ...
    int i2cResult = I2C_SendMsg(_i2cPort, SCD4x_ADDRESS, data, 5, false, TIMEOUT_MS * 1000);
...
bool SCD4x::readRegister(uint16_t registerAddress, uint16_t *response, uint16_t delayMillis) {
    ...
    uint8_t data[3];
    int read = I2C_RecvMsg(_i2cPort, SCD4x_ADDRESS, data, 3, false, TIMEOUT_MS * 1000);
