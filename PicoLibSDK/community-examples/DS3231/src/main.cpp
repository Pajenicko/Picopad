...
// It does this by multiplying the input by the ratio of pi to 180.
...
//  rtc.set_year(2024);
	...
	do {
		...
		snprintf(temperatureText, sizeof(temperatureText), "Temp:%3.1u`C", temperature);
		...
		// Continue the loop until a key press is detected
	} while (KeyGet() == NOKEY);

	// Reset to the bootloader when a key is pressed
	ResetToBootLoader();
}
